...
const validateState = (input: unknown): OrgToolsState => {
  try {
    ...
    throw new StateRepositoryError("invalid_state", "State is invalid.", { cause: error });
  }
};
...
const hasExactStateColumns = (database: DatabaseSync): boolean => {
  const columns = (
    database.prepare("PRAGMA table_info(application_state)").all() as Array<{ name: string }>
  )
    .map((row) => row.name)
    .sort();
  return (
    columns.join("\0") ===
    ["created_at", "id", "organization_json", "revision", "ui_json", "updated_at"].sort().join("\0")
  );
};
...
export class StateRepository {
  ...
  private initialize(): void {
    ...
    if (
      ...
      toNumber(versionRow?.user_version ?? 0) !== SCHEMA_VERSION ||
