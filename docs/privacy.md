...
The GitHub Pages site is static product documentation, not a hosted Org Tools runtime. Its ignored
artifact is generated from reviewed English copy, `docs/screenshot-demo.json`, and the corresponding
synthetic PNGs. It contains no workspace JSON, SQLite database, project endpoint, writable form,
browser persistence, JavaScript, telemetry, or remote asset. Browsing it never enters the
organization-data path. Functional use follows the repository instructions and starts a separate
loopback runtime on the user's computer.

`pnpm pages:check` verifies artifact shape, exact screenshot parity, local resources, repository-only
referrer-protected links, and the absence of application storage or API references. The broader
`pnpm public:check` scans the same generated bytes with the production build and tracked files.
...
generated Pages showcase for non-portable paths, unexpected non-English source text, unsafe remote
