...
permitted to contain Cyrillic product copy. Bundled import interfaces and examples SHALL be
synthetic, English-keyed, and validated by the production parsers.

#### Scenario: Continuous validation
- **WHEN** the CI workflow runs on a clean checkout
- **THEN** install, lint, typecheck, locale and import example tests, unit tests, build, OpenSpec validation, localized browser smoke, and public-safety checks complete successfully

#### Scenario: Screenshot generation
- **WHEN** the screenshot command runs against the production build
- **THEN** deterministic English PNG screenshots are generated from local synthetic data including revised empty and import guidance surfaces
