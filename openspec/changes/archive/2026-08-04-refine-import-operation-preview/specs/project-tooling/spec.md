...
- **THEN** install, lint, typecheck, locale, scoped state, virtualized import preview, generic JSON mapping, tag packing, editor and PNG geometry, data download, unit, build, OpenSpec, localized browser smoke, and public-safety checks complete successfully
...
- **THEN** deterministic English PNG screenshots cover separated state-import operations, hierarchy and Employee preview cards, generic JSON mapping, hidden tag dates, wrapping tags, and the shadow-free monochrome logo
