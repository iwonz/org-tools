...
JSON only, and publication rules SHALL be expressed as general repository checks.

#### Scenario: Continuous validation
- **WHEN** the CI workflow runs on a clean checkout
- **THEN** install, lint, typecheck, locale, scoped state, virtualized import preview, borderless interface chrome, generic JSON mapping, tag packing, editor and PNG geometry, data download, unit, build, OpenSpec, localized browser smoke, and public-safety checks complete successfully
...
- **THEN** deterministic English PNG screenshots cover borderless shell, surface and dialog chrome, preserved meaningful outlines, separated state-import operations, hierarchy and Employee preview cards, generic JSON mapping, hidden tag dates, wrapping tags, and the shadow-free monochrome logo
