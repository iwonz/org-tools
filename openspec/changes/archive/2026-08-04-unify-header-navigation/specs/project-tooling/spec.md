...
- **THEN** install, lint, typecheck, locale, unified responsive header, scoped state, virtualized import preview, borderless interface chrome, generic JSON mapping, tag packing, editor and PNG geometry, data download, unit, build, OpenSpec, localized browser smoke, and public-safety checks complete successfully
...
- **THEN** deterministic English PNG screenshots cover the wordmark-free unified header, flag-only locale trigger, responsive transfer actions, borderless chrome, preserved meaningful outlines, state import, generic JSON mapping, hidden tag dates, and wrapping tags
