...
The application SHALL not transmit workspace state, generic rows, state projections, detached
...
- **WHEN** a user maps JSON or CSV, appends or replaces a state projection, edits tag dates, opens calendar dialogs, saves, searches, analyzes, exports, and changes locale
...
- **WHEN** a user cancels mapping, state projection, destructive replacement, tag-date edit, Save dialog, avatar source, or crop
