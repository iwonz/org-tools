...
parser without a separate import schema or legacy migrations.

#### Scenario: Continuous validation
- **WHEN** the CI workflow runs on a clean checkout
- **THEN** install, lint, typecheck, locale, scoped state, generic mapping, tag packing, editor and PNG geometry, export, unit, build, OpenSpec, localized browser smoke, and public-safety checks complete successfully
...
- **THEN** the obsolete `org-tools-import` kind, format examples, version fields, and compatibility code are absent

#### Scenario: Screenshot generation
- **WHEN** the screenshot command runs against the production build
- **THEN** deterministic English PNG screenshots cover scoped Save/import, generic mapping, hidden tag dates, wrapping tags, and the shadow-free logo
