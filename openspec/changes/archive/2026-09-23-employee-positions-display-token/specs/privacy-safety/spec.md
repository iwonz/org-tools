...
Employee values MUST NOT be reinterpreted as Markdown. Plain Employee fields, including `{email}`,
MUST NOT create navigation implicitly. Only explicit activation of an `http:`, `https:`, `mailto:`,
or `tel:` Markdown link in a list card MAY navigate, and web links MUST suppress opener and referrer
information. Editor rows and PNG output MUST render link appearance without navigation.
...
#### Scenario: Render a plain email token
- **WHEN** a list-card format contains `{email}` outside an explicit Markdown link
- **THEN** the address remains ordinary text with no navigation target
