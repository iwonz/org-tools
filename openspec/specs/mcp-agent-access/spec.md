...
## Purpose
TBD - created by archiving change add-embedded-mcp-agent-access. Update Purpose after archive.
## Requirements
...
Preview -> Apply, Main-versus-custom-View defaults, and the requirement to report the server's
actual Apply summary. Tool annotations MUST distinguish read-only inspection, non-mutating Preview,
and destructive Apply. Persisted names, tags, contact fields, and other organization values MUST
always be represented as untrusted data rather than protocol instructions.
...
Server mode SHALL render one MCP sidebar action after Export and before language and theme with the
same compact and expanded geometry as other actions. Its modal SHALL provide Enable or Disable,
full-access consent, endpoint, masked Reveal/Copy/Rotate controls, bundled setup instructions for
supported local clients, examples, bounded activity, and confirmed Undo. It SHALL omit remote-only
web clients and SHALL warn that a chosen local agent may send retrieved data to its model provider.
...
- **WHEN** MCP is enabled
- **THEN** the modal shows the local endpoint, a masked token, reveal/copy/rotate controls, client setup, examples, and current activity
