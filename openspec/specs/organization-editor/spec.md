...
#### Scenario: Unified Analytics surface
- **WHEN** Analytics is ready
- **THEN** all six sortable virtualized groups and drill-down actions remain available inside one outer island in two desktop columns and one narrow-screen column
- **AND** quiet nested backgrounds plus hover or focus feedback separate compactly spaced groups and borderless rows without horizontal divider lines
