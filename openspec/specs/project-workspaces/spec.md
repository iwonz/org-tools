...
Organization changes SHALL remain dirty until explicit Save, SHALL support Ctrl+S and Cmd+S, and
SHALL be written as one parsed full-state snapshot in an atomic SQLite transaction. Save SHALL send
the expected state revision and increment the stored revision only after a successful write. Import
SHALL make organization state dirty; Export SHALL use the current live state regardless of whether
it has been saved.

#### Scenario: Explicit Save
- **WHEN** a user saves a dirty project with the current revision
- **THEN** the exact validated organization state is committed, the revision increments once, and
  the project becomes clean unless a newer local change occurred during the request

#### Scenario: Invalid state Save
- **WHEN** the submitted state fails the production `OrgToolsState` parser
- **THEN** the stored state and revision remain unchanged and the client remains dirty

#### Scenario: Unsaved project navigation
- **WHEN** a user switches project or follows internal project navigation while organization data is
  dirty
- **THEN** Save, Discard, and Cancel choices are offered and navigation occurs only after the chosen
  safe outcome
